...
export function isFeedgenConfigured() {
    return Boolean(baseUrl && secret);
}
export async function pushFeedContents(feedUri, postUris) {
    if (!baseUrl || !secret) {
        throw new Error('Feed generator URL and secret are required to publish feed contents. Set VITE_FEEDGEN_URL and VITE_FEEDGEN_SECRET in .env.');
    }
    ...
    if (!response.ok) {
        ...
        throw new Error(`Feed generator returned ${response.status}: ${text || response.statusText}`);
    }
}
export async function removeFeedContents(feedUri) {
    if (!baseUrl || !secret)
        return;
